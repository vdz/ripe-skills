...
2. **Reflects what's shown** — store what the UI displays, not raw API shapes or intermediate computations; those get formatted in the api layer or derived in selectors, so state stays a clean mirror of the screen. — in the branch's own types, never a client's (see [api.md → The App's Own Types](api.md#the-apps-own-types))
